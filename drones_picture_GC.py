...
model.load_state_dict(torch.load('./fastRCNN/fastRCNN_GC_SGD_Aug.pth',map_location=device))
model.eval()
model.to(device)
...
image = read_image("./data_drones/Spectral_Augmented/Green_Channel/Test_Images/Image_010.jpg")
...
output_image = draw_bounding_boxes(image, pred_boxes, pred_labels,
                                    colors="red",font_size=32)
image_numpy = output_image.numpy()
if image_numpy.shape[0] == 3:
    image_numpy = image_numpy.transpose(1, 2, 0)

plt.figure(figsize=(12, 12))
#plt.plot(output_image.permute(1, 2, 0))

plt.imshow(image_numpy)
plt.show()

plt.imsave('Results_test_GC.png', image_numpy)
